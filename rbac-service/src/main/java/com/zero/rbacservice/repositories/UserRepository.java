...
public interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Modifying
    @Query("update User u set u.status=com.zero.rbacservice.model.enums.Status.INACTIVE where u.id = :id")
    int disableUser(UUID id);

    @Modifying
    @Query("update User u set u.status=com.zero.rbacservice.model.enums.Status.ACTIVE where u.id = :id")
